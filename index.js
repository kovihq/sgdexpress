...
import got from 'got';
import { xmlConductor, xmlVehicle, url, clean, valid, cleanCnh, validCnh } from './lib/util';
...
class Service {
  constructor(code, user, pass) {
    ...
    this.user = user;
    this.pass = pass;
    this._parseString = promisify(parseString);
    this.headers = {
  ...
  async vehicle(plate) {
    ...
    try {
      ...
      const resParse = await this._parseString(res.body, { explicitArray: false, trim: true });
  ...
  async conductor(cnh, cpf) {
    ...
    try {
      ...
      const res = await got.post(url.conductor, { headers: this.headers, body });
      const resParse = await this._parseString(res.body, { explicitArray: false, trim: true });
